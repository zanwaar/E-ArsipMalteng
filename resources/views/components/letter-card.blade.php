    ...
    <div class="card-header pb-0">
        <div class="d-flex justify-content-between flex-column flex-sm-row">
            <div class="card-title">
                <h5 class="text-nowrap mb-0 fw-bold">{{ $letter->nomor_dokument }}
                    @if($letter->type == 'bidang')
                    || <span class="badge bg-label-dark">{{ $letter->bidang }}</span>
                    @endif
                </h5>
                <small class="text-black">
                    {{ $letter->type == 'incoming' ? $letter->from : $letter->to }} |
                    <span class="text-secondary">{{ __('model.letter.nomor_agenda') }}:</span> {{ $letter->nomor_agenda }}
                    |
            ...
            <div class="card-title d-flex flex-row">
                <div class="d-inline-block mx-2 text-end text-black">
                    <small class="d-block text-secondary">{{ __('model.letter.tgl_surat') }}</small>
                    {{ $letter->formatted_tgl_dokument }}
                </div>
                ...
                <div class="dropdown d-inline-block">
                    ...
                    @elseif ($letter->type == 'bidang')
                    <div class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdown-{{ $letter->type }}-{{ $letter->id }}">
                        @if(!\Illuminate\Support\Facades\Route::is('*.show'))
                        <a class="dropdown-item" href="{{ route('transaction.bidang.show', $letter) }}">{{ __('menu.general.view') }}</a>
                        @endif
                        <a class="dropdown-item" href="{{ route('transaction.bidang.edit', $letter) }}">{{ __('menu.general.edit') }}</a>
                        <form action="{{ route('transaction.bidang.destroy', $letter) }}" class="d-inline" method="post">
                            @csrf
                            @method('DELETE')
                            <span class="dropdown-item cursor-pointer btn-delete">{{ __('menu.general.delete') }}</span>
                        </form>
                    ...
                    <div class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdown-{{ $letter->type }}-{{ $letter->id }}">
                        ...
                        <form action="{{ route('transaction.outgoing.destroy', $letter) }}" class="d-inline" method="post">
                            @csrf
                            @method('DELETE')
                            <span class="dropdown-item cursor-pointer btn-delete">{{ __('menu.general.delete') }} f</span>
                        </form>
                    </div>
                    @else
                    <div class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdown-{{ $letter->type }}-{{ $letter->id }}">
                        <a class="dropdown-item" href="#">Tidak ada Opsi</a>
                    </div>
                    @endif
                </div>
                @else
        ...
        <div class="d-flex justify-content-between flex-column flex-sm-row">
            ...
            <div>
                ...
                <a href="{{ $attachment->path_url }}" target="_blank">
                    ...
                    <i class="bx bxs-file-pdf display-1 cursor-pointer text-danger"></i>
