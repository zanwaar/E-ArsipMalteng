        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $user->email }}</td>
                    <td>{{ $user->phone }}</td>
                    <td><span class="badge bg-label-primary me-1">{{ __('model.user.' . ($user->is_active ? 'active' : 'nonactive')) }}</span>
                    </td>
                    <td>
                        <button class="btn btn-info btn-sm btn-edit" data-id="{{ $user->id }}" data-name="{{ $user->name }}" data-jabatan="{{ $user->jabatan }}" data-email="{{ $user->email }}" data-phone="{{ $user->phone }}" data-role="{{ $user->role }}" data-active="{{ $user->is_active }}" data-bs-toggle="modal" data-bs-target="#editModal">
    ...
    <div class="modal-dialog">
        <form class="modal-content" method="post" action="{{ route('user.store') }}">
            ...
            <div class="modal-body">
                ...
                <x-input-form name="jabatan" :label="__('model.user.jabatan')" />
    ...
    <div class="modal-dialog">
        <form class="modal-content" method="post" action="">
            ...
            <div class="modal-body">
                ...
                <x-input-form name="phone" :label="__('model.user.phone')" />
                <label for="role" class="form-label">Role</label>
                <select class="form-select role mb-3" id="role" name="role">
                    <option value="staff">
                        Pengguna
                    </option>
                    <option value="operator">
